...
void ProceduralFirstRooms::GenerateRoomFloor(void)
{
    ...
    for (int i = 0; i < 5; ++i) {
        LoadModule("modules/" + std::to_string(random) + "/floor/" +
                       floorModules[i],
                   &floor);
    }
}
...
void ProceduralFirstRooms::GenerateRoomWalls(int roomId)
{
    ...
    for (int i = 4; i > 0; i--) {
        j = pow(10, i);
        if (roomId >= j) {
            LoadModule("modules/fixed/wall/doored/" + wallsModules[i - 1],
                       &walls);
            roomId -= j;
        } else {
            LoadModule("modules/fixed/wall/doorless/" + wallsModules[i - 1],
                       &walls);
        }
    }
}
...
bool ProceduralFirstRooms::GenerateRoomDetails(void)
{
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (prob > 75) {
            ...
            LoadModule("modules/" + std::to_string(random) + "/details/" +
                           detailsModules[i],
                       &details);
            hasDetails = true;
        }
...
void ProceduralFirstRooms::WriteRoom(int roomId, bool hasDetails)
{
    ...
    if (hasDetails)
        out << "20,11,3," << std::endl
            << std::endl;
    else
        out << "20,11,2," << std::endl
            << std::endl;
